...
(define-read-only (get-vote-stats (proposal-id uint))
  (default-to
    { total-yes: u0, total-no: u0, total-voters: u0, finalized: false }
    (map-get? vote-stats { proposal-id: proposal-id })
  )
)

(define-read-only (get-vote-weight (vault-id uint) (voter principal))
  (default-to
    { weight: u1 }
    (map-get? vote-weights { vault-id: vault-id, voter: voter })
  )
)

(define-read-only (has-voter-voted (proposal-id uint) (voter principal))
  (contract-call? .proposal-engine has-voted proposal-id voter)
)
...
(define-public (cast-vote (proposal-id uint) (vote bool))
  (let
    ...
    (asserts! (contract-call? .multisig-vault is-member vault-id caller) ERR-NOT-MEMBER)
    (asserts! (contract-call? .proposal-engine is-proposal-active proposal-id) ERR-VOTING-CLOSED)
...
(define-public (set-vote-weight (vault-id uint) (voter principal) (weight uint))
  (let
    (
      (caller tx-sender)
    )
    (asserts! (contract-call? .multisig-vault is-vault-owner vault-id caller) ERR-NOT-AUTHORIZED)
    (asserts! (contract-call? .multisig-vault is-member vault-id voter) ERR-NOT-MEMBER)

    (map-set vote-weights
      { vault-id: vault-id, voter: voter }
      { weight: weight }
    )
...
(define-public (finalize-votes (proposal-id uint))
  (let
    (
      (stats (get-vote-stats proposal-id))
    )
